...
// ============================================================
bool SIM800::wakeUp() {                                         // Check if module is not sleeping (AT+CSCLK=2)
    outBuilder(EXE, P(""), P("AT"), false);                         // If module is sleeping, send AT commands until wake up
    write(ioBuffer);
    clearBuffer(); 
    delay(100);                                                  
    outBuilder(EXE, P(""), P("AT"), false);                             
    print(ioBuffer);
    ...
    switch (type) {
        ...
        case EXE:                                                           // AT method EXE = AT command type EXECUTE in official manual
            if(strlen(ioBuffer) + strlen(param) <= DEF_BUFFER_SIZE) {       // If the sum of AT cmd and param length is greater than the size of ioBuffer, indicate buffer overflow,
                strcat_P(ioBuffer, param);                                    // else, concat param to outgoing AT cmd if type == SET or EXE
            } else {
                clearBuffer();
...
bool SIM800::endOfTx(uint32_t i) {                                          // Method responsible for detecting the end strings "OK" and "ERROR".
    if (ioBuffer[i-2] != 'K' && ioBuffer[i-2] != 'R' && ioBuffer[i-2] != 'D')                       // If detectEndStr is true, this function is run each time the currently read
        return false;                                                       // char == 10 (line terminator). If it finds either "10, 79, 75, 13, 10" or 
    if (ioBuffer[i-3] != 'O' && ioBuffer[i-3] != 'A')                                               // "10, 69, 82, 82, 79, 82, 13, 10" in the ioBuffer, it returns true to indicate the end of transmission.
        return false;                                                       // This enables the read() method to exit the read loop earlier before it times out
    if (ioBuffer[i-4] != 'R' && ioBuffer[i-4] != 'O' && ioBuffer[i-4] == 10)                        // and to speed up the communication between the two devices.
        return true;
    if (ioBuffer[i-5] != 'R' && ioBuffer[i-5] != 'L') 
        return false;
    if (ioBuffer[i-6] != 'E' && ioBuffer[i-6] != 'N') 
        return false;
    if (ioBuffer[i-7] == 10 || ioBuffer[i-10] == 10) 
        return true;
