...
SoftwareSerial simCom (SIM800_RX_PIN, SIM800_TX_PIN);
...
// ============================================================
void SIM800::send(const char* command) 
{   
    outBuilder(EXE, P(""), command, false);
    print(ioBuffer);                                                  
}

// ============================================================
void SIM800::send(CmdType type, const char* command) 
{   
    outBuilder(type, "", command);
    print(ioBuffer);                                                  
}

// ============================================================
void SIM800::send(CmdType type, const char* command, const char* params) 
{   
    outBuilder(type, params, command);
    print(ioBuffer);                                                  
}
// ============================================================
void SIM800::sendSMS(const char* addr_len, const char* txt_pdu) {
    outBuilder(SET, P("1"), P("CMGF"));
    print(ioBuffer);
    clearBuffer();
    strcat_P(ioBuffer, P("AT+CMGS=\""));
    strcat(ioBuffer, addr_len);
    strcat_P(ioBuffer, P("\"\r"));
    write(ioBuffer);
    delay(100);
    clearBuffer();
    strcat(ioBuffer, txt_pdu); 
...
// ============================================================
void SIM800::deleteSMS(const char* index) {
    clearBuffer();
    strcat_P(ioBuffer, P("AT+CMGD="));
    strcat(ioBuffer, index);
    print(ioBuffer);
}
...
void SIM800::resetTimeout() {                                   // Resets read() max time limit to the millisecond value defined in SIM800.h file.
    timeLimit = 1000 * (unsigned long) DEF_TIME_LIMIT;                          // Multiplies the provided millisecond value with 1000 to get microseconds, 
}                                                               // because the internal timeOut method uses micros() to calculate timeout.
